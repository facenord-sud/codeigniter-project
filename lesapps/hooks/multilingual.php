...
//	}
    ...
    public function set_route() {
        // YVES : J'ajoute une global $_USED_PROTOCOL, $_CURRENT_TAG pour ajouter aux autres
        // Defined global variable for the routes
        global $_ROUTE, $_ROUTING, $_PRE_ROUTE, $_CURRENT_LANGUAGE, $_USED_PROTOCOL, $_CURRENT_TAG;

        // Find the correct current language
        $this->used_protocol = $_USED_PROTOCOL = $this->_get_language();
        $_CURRENT_LANGUAGE = $this->current_language[0];
        $_CURRENT_TAG = $this->current_language[1];

        // Get pre uri informations
        $_PRE_ROUTE = $this->_find_pre_uri();

        // Load route language file.
        // modifié pour avoir un seul fichier de routes
        require APPPATH . 'language/route_lang.php';
        // Numa: j'ai modifié pour inclure les routes générées par le hook RoutingHook
        $lang['route'] = array_merge($_ROUTING['route'], $lang['route']);
        ...
        foreach ($lang['route'] as $key => $array) {
            foreach ($array as $uri => $ruri) {
                ...
                // J'ajoute cette ligne pour que les routes sans langues soit aussi trouvée (à laisser)
                $route[trim($uri, '/')] = $ruri;
            }
        }
     ...
     * Route modified
     *
     * Si la langue a été trouvé grâce au Browser, on l'ajoute dans l'url.
     *
     * @access	public
     * @param	none
     * @return	void
    ...
    public function redirect_route() {
        // Defined global variable for the routes
        global $_CURRENT_TAG, $_USED_PROTOCOL;

        $curent_uri = trim($_SERVER['REQUEST_URI'], '/');
        // Ajouté juste pour le local (enlever le collaide2 au début de l'adresse)
        $curent_uri = ltrim($curent_uri, "collaide2");

        if ($_USED_PROTOCOL == 'BROWSER') {
            // Rediriger vers la meme page mais en spécifiant la langue
            redirect('/' . $_CURRENT_TAG . $curent_uri, 'location', 301);
        }
    }
