...
class MY_Loader extends CI_Loader {
    ...
    /**
     * permet l'utilisation de l'orm
     * @author Numa de Montmollin <facenord.sud@gmail.com>
     */
    public function orm() {
        $CI = & get_instance();
        if (isset($CI->factory2)) {
            return;
        }
        $this->model('orm/factory/factory2', 'factory2');
        $this->model($CI->factory2->getPlugin()->getDbDriver(), 'driver');
        $this->model($CI->factory2->getPlugin()->getDbForge(), 'forge');
        $this->model($CI->factory2->getPlugin()->getQueryBuilder(), 'query');
        $this->model($CI->factory2->getPlugin()->getTreeBuilder(), 'tree');
        $this->model('orm/dmo', 'dmo');
     
        $CI->forge->setDriver($CI->driver);
        $CI->query->setDBForge($CI->forge);
        $CI->dmo->setQueryBuilder($CI->query);
        $CI->tree->setDmo($CI->dmo);
        $CI->tree->setQueryBuilder($CI->query);
        $CI->query->setTreeBuilder($CI->tree);
        
    }

    /**
     * permet le chargement de formulaires
     * le premier paramètre doit être de la forme: dossier/nom_du_formulaire
     * 
     * @param string $form le nom de la forme
     * @author Numa de Montmollin <facenord.sud@gmail.com>
     */
    public function form($form, $name = '') {
        ...
        $pathArray = explode('/', $form);
        if (count($pathArray) < 2) {
            show_error("the name of the entity '$form' is invalide");
        }
        ...
        $this->model($dir . '/form/' . $fileName . '_form', $name);
        $CI->twiggy->set($name, $CI->$name);
    }

    /**
     * permet le chargement d'e formulaires'entitées
     * le premier paramètre doit être de la forme: dossier/nom_de_l'entité
     * 
     * @param string $entity le nom de l'entitée
     * @param string $name le nom à utiliser
     * @author Numa de Montmollin <facenord.sud@gmail.com>
     */
    public function entity($entity, $name='') {
        $CI = & get_instance();
        if (empty($name)) {
            $name = $entity;
        }
        ...
        $pathArray = explode('/', $entity);
        if (count($pathArray) < 2) {
            show_error("the name of the entity '$entity' is invalide");
        }
        $fileName = $pathArray[count($pathArray) - 1];
        unset($pathArray[count($pathArray) - 1]);
        $dir = implode('/', $pathArray);
        $this->model($dir.'/entity/'.$fileName.'_entity', $name);
    }
