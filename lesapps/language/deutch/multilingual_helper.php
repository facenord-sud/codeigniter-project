 ...
 * @return	string
 */
// J'ai ajouter FALSE à $params par défault.
if ( ! function_exists('_t')) 
{
    function _t($str, $params = FALSE, $segment = '%s')
    {
...
//                            }
                        // J'ai virer le haut, car il met le fichier route_lang.php dans la langue.
                            if( file_exists(APPPATH.'language/route_lang.php') )
                                {
                                        require_once APPPATH.'language/route_lang.php';
                                        if( isset($lang['route'])) { $CI->config->set_item('language_route_', $lang['route']); }

                                        $route = $CI->config->item('language_route_');
                                        $pre_uri = $_PRE_ROUTE[$idiom];
                                }
                    }
            }
